...
// Google par défaut du téléphone (souvent Gmail perso) → crée un compte Firebase
// Auth orphelin sans doc équipe → Accès refusé. Naomi a vécu ça.
// Maintenant : PIN par défaut pour tout le monde (équipe terrain), bouton
// Google replié sous un lien discret "Connexion administrateur" pour les rares
// admins qui veulent encore l'utiliser.
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-gray-800 rounded-2xl p-6 shadow-xl space-y-4">
          ...
          {/* Bouton Google replié sous un lien discret. Caché par défaut pour
              les rôles terrain (preparateur/chauffeur/monteur) qui doivent
              utiliser nom + PIN — sinon Android pique le Gmail perso et
              crée un compte Firebase orphelin → "Accès refusé" garanti. */}
          {!showGoogle ? (
            <button
              type="button"
              onClick={() => setShowGoogle(true)}
              className="w-full text-center text-[11px] text-gray-500 hover:text-gray-400 underline-offset-2 hover:underline"
            >
            ...
            <>
              ...
              <p className="text-[10px] text-amber-300/80 text-center">
                ⚠ Réservé aux admins. Utilise ton compte Workspace, pas un Gmail perso.
              </p>
            </>
          )}
        </div>
